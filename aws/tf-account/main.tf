...
# Assume we require a NCC per workspace allowing for better isolation and tailored configuration per workspace
module "network_connectivity_configuration" {
  ...
  region          = var.region
  resource_prefix = var.workspace_resource_prefix
}

# Create a Network Policy
# Assume we require a NP per workspace allowing for better isolation and tailored configuration per workspace
...
module "log_delivery" {
  ...
  databricks_account_id = var.databricks_account_id
  resource_prefix       = var.account_resource_prefix
...
module "restrictive_root_bucket" {
  ...
  root_s3_bucket        = "${var.workspace_resource_prefix}-workspace-root-storage"
}
